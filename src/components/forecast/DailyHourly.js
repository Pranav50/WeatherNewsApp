...
const DailyHourly = (props) => {
  ...
  const {dailyforecast, hourlyforecast, timezone, unit, time, isDayMode} = props

  console.log('PROPS', isDayMode)
    ...
    <div className={classes.root}>
      ...
      >
        <TabPanel value={value} index={0} dir={theme.direction}>
            <ForecastDays dailyforecast={dailyforecast} unit={unit} time={time} />
        </TabPanel>
        <TabPanel value={value} index={1} dir={theme.direction}>
            <ForecastDays timezone={timezone} unit={unit}  hourlyforecast={hourlyforecast} time={time} />
        </TabPanel>
      </SwipeableViews>
    </div>
  );
